...
# --- THE HYBRID SETTINGS ---
CASCADE_DIST = 0.6  # METERS: Rays < 0.6m are Physics. Rays > 0.6m are AI.
LOOKAHEAD = 10.0  # FRAMES: How far into the future the Oracle sees.

print(f"Running ULTIMATE Neural Ray Tracer on: {DEVICE}")


# --- 1. MODEL (Optimized for Cascades) ---
# We use the smaller, faster brain because it doesn't need to learn sharp corners.
class HashEmbedder(nn.Module):
    def __init__(self, num_levels=8, base_res=16, max_res=256, log2_hashmap_size=12):
        super().__init__()
        self.resolutions = [
            int(
                base_res
...
# --- 4. INTEGRATOR (Cascade + Oracle) ---
def trace_hybrid(sc, md, ro, rd):
    # 1. Primary Hit (Always Physics)
    t, n, c, m = sc.intersect(ro, rd)
    p = ro + rd * t.unsqueeze(1)

    # 2. Direct Light (Physics)
    ld = sc.light - p
    ...
    # CASCADE LOGIC: Near Field (<0.6m) vs Far Field (>0.6m)
    is_near = mb & (tb < CASCADE_DIST)

    # AI Estimate (Base Layer)
    pb = p + r * tb.unsqueeze(1)
    indirect = md(pb, nb)

    # Physics Overlay (Detail Layer)
    if is_near.any():
        ldb = sc.light - pb
        ...
        # If Near: Light = Direct(B) + AI(B)
        indirect[is_near] = db_light[is_near] + md(pb, nb)[is_near]  # Recurse once

    return p, n, c, direct, (direct + indirect) * c, m


# --- 5. RENDER LOOP (Oracle Logic) ---
def render(sc, cam, md, opt):
    # A. Current View
    ro, rd = cam.rays()
    p, n, c, d, tgt, v = trace_hybrid(sc, md, ro, rd)
    l_curr = nn.MSELoss()(md(p, n)[v], tgt[v])

    # B. Oracle View (Future)
    is_act = (cam.v.norm() > 1e-3) | (abs(cam.av) > 1e-3)
    orc_img = np.zeros((RES_Y, RES_X, 3))

    if is_act:
        # Project Future Camera
        ...
        fp_s, fn_s, _, _, ft_s, fv_s = trace_hybrid(sc, md, fo[idx], fd[idx])

        # Weighted Training
        l_fut = nn.MSELoss()(md(fp_s, fn_s)[fv_s], ft_s[fv_s])
        loss = l_curr + l_fut * 5.0  # Prioritize Future!

        # Full Res Oracle for Debug Display
        with torch.no_grad():
...
def main():
    ...
    print("--- ULTIMATE RENDERER ---")
    print("KEYS: W/A/S/D to Move, Q/E to Turn")
    ...
    while True:
        ...
        cv2.putText(
            ui,
            f"Cascade+Oracle: {'ON' if act else 'IDLE'}",
            (10, 20),
            0,
            0.5,
            (0, 255, 0),
