...
CASCADE_DIST = 0.6
LOOKAHEAD = 10.0  # Oracle is BACK
TRAIN_BATCH = 2048
GLOBAL_BATCH = 512  # Memory Rays
ORACLE_BATCH = 512  # Future Rays

print(f"Running ULTIMATE NRC (Manifold + Memory + Oracle) on: {DEVICE}")
...
class Camera:
    ...
    def update(self, k):
        ...
        # Velocity Tracking (Crucial for Oracle)
...
# --- 5. RENDER (THE TRINITY) ---
def render(sc, cam, md, opt):
    is_moving = (cam.v.norm() > 0.001) or (abs(cam.av) > 0.001)

    # 1. BATCH GENERATION
    # A. Camera (The Present)
    ...
    rd_cam = torch.stack([rx * c - rz * s, ry, rx * s + rz * c], -1)
    rd_cam = rd_cam / rd_cam.norm(dim=-1, keepdim=True)
    ro_cam = cam.p.expand_as(rd_cam)

    # B. Global Memory (The Past/Stability)
    ...
    # C. Oracle (The Future) - RESTORED
    if is_moving:
        # Predict where we will be in 10 frames
        fp = cam.p + cam.v * LOOKAHEAD
        fy = cam.y + cam.av * LOOKAHEAD

        # Random rays from that future perspective
        # We reuse the camera generation logic but with future coords
        frx, fry, frz = (
            rx[:ORACLE_BATCH],
            ry[:ORACLE_BATCH],
            -torch.ones_like(rx[:ORACLE_BATCH]),
        )
        fangle = torch.tensor(fy, device=DEVICE)
        fc, fs = torch.cos(fangle), torch.sin(fangle)
        frd = torch.stack([frx * fc - frz * fs, fry, frx * fs + frz * fc], -1)
        frd = frd / frd.norm(dim=-1, keepdim=True)
        fro = fp.expand_as(frd)

        # Combine All 3: [Present, Past, Future]
        ro = torch.cat([ro_cam, gro, fro], dim=0)
        rd = torch.cat([rd_cam, grd, frd], dim=0)
    else:
        # Combine 2: [Present, Past]
        ro = torch.cat([ro_cam, gro], dim=0)
        rd = torch.cat([rd_cam, grd], dim=0)

    # 2. NEIGHBOR GENERATION (Manifold Consistency)
    # We perturb EVERY ray (Current, Global, and Oracle) to enforce smoothness everywhere.
    ro_n = ro + (torch.randn_like(ro) * 0.01)

    # 3. TRACE & TRAIN
    p, n, c, d, tgt, v = trace_hybrid(sc, md, ro, rd)
    p_n, n_n, _, _, _, v_n = trace_hybrid(sc, md, ro_n, rd)

    train_tgt = tgt / (c + 1e-4)
    pred_main = md(p, n)
    pred_neighbor = md(p_n, n_n)

    # Loss A: Physics Accuracy
    loss_phys = ((pred_main[v] - train_tgt[v]) ** 2).mean()

    # Loss B: Spatial Consistency (Smoothness)
    normal_dot = (n[v] * n_n[v]).sum(dim=1)
    is_same = normal_dot > 0.95
    loss_smooth = ((pred_main[v][is_same] - pred_neighbor[v][is_same]) ** 2).mean()

    total_loss = loss_phys + (loss_smooth * 0.1)
    ...
    return img, total_loss.item(), is_moving
...
def main():
    ...
    print("--- NRC FINAL ---")
    print("1. Manifold Consistency (No Checkerboard)")
    print("2. Global Memory (No Green Screen)")
    print("3. Oracle Lookahead (No Lag)")
    ...
    while True:
        ...
        cam.update(k)
        img, l, act = render(sc, cam, md, opt)

        ui = cv2.cvtColor(img, cv2.COLOR_RGB2BGR)
        status = "ORACLE ACTIVE" if act else "IDLE"
        cv2.putText(
            ui,
            status,
            (10, 20),
            ...
            0.5,
            (0, 255, 0) if act else (0, 0, 255),
            1,
        )
        cv2.imshow("NRC Final", ui)
