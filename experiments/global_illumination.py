...
prev_image = ti.Vector.field(3, dtype=ti.f32, shape=(RES_X, RES_Y))  # History Buffer
...
class Scene:
    ...
    def intersect_scene(self, ro, rd):
        ...
        # Cube
        c = ti.Vector(
            [ti.sin(time_val[None]) * 0.5, -0.6, ti.cos(time_val[None]) * 0.5]
        )
        b_min = c - 0.4
        b_max = c + 0.4
...
def neural_inference():
    for x, y in raw_image:
        if mat_id[x, y] == -1:
            ...
            # Analytic Direct (16 Samples for High Quality Penumbra)
...
def temporal_accumulate():
    ...
    for x, y in final_image:
        ...
        # Detect large changes (Shadow moving)
        diff = (cur - old).norm()

        # Adaptive Alpha:
        # If difference is small -> Blend heavily (0.05) -> No noise
        # If difference is large -> Blend lightly (0.5) -> No trails
        alpha = 0.05
        if diff > 0.1:
            alpha = 0.5
        if frame_idx[None] < 2:
            alpha = 1.0  # First frame
...
def main():
    ...
    gui = ti.GUI("Premium Neural Oracle", res=(RES_X, RES_Y))
    ...
    while gui.running:
        ...
        neural_inference()
        temporal_accumulate()  # Fixes grain and flicker
