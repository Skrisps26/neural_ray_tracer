...
print(f"Running STABLE NRC on: {DEVICE}")


# --- 1. MODEL (Higher Resolution) ---
class HashEmbedder(nn.Module):
    def __init__(self, num_levels=12, base_res=16, max_res=1024, log2_hashmap_size=16):
        # Increased to 16 (65k parameters) to reduce grid collisions/checkerboarding
        super().__init__()
        self.resolutions = [
            int(
                base_res
...
class TemporalDenoiser:
    ...
    def apply(self, new_frame, v_linear, v_angular):
        ...
        # BALANCED WEIGHTS:
        # Turning: 0.10 (Small history to hide raw noise, but mostly new)
        # Moving:  0.20 (Enough history to smooth checkerboard, not enough to blur)
        # Still:   0.90 (Perfect smooth)
        weight = 0.10 if is_turning else (0.20 if is_moving else 0.90)
...
def render(sc, cam, md, opt, denoiser):
    ...
    # Reduced Jitter (0.005 -> 0.002) to stop Grid Shimmering
    ro_cam = ro_cam + (torch.randn_like(ro_cam) * 0.002)
    ...
    # Reduced Importance Weight (10.0 -> 4.0) to stop Shadow Checkerboarding
    weights = 1.0 + (error * 4.0).detach()
...
def main():
    ...
    while True:
        ...
        cv2.putText(ui, f"Stable Mode", (10, 20), 0, 0.5, (0, 255, 0), 1)
        cv2.imshow("Stable NRC", ui)
